...
@router.get("/studios/{studio_id}/banner", tags=["Studio"])
...
async def upload_room_images(
    ...
    await room_service.update_by_id(
        room.id, {"images_filenames": new_filenames}
    )
    urls = [
        await file_service.get_url_by_name(filename) for filename in filenames
    ]
    return urls
