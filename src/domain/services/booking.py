...
class BookingService(
    ...
    async def get_slots_by_user_id(
    ...
    async def patch_slot(
        ...
        if (
            ...
            raise HTTPException(
                ...
                detail="User does not have permission to cancel the slot",
            )

        slot = await self._repository.get_one_with_room_relation(
            self._model.id == slot_id
        )
    ...
    async def remove(self, slot_id: uuid.UUID, user_id: int) -> None:
        if not await self.check_user_permission(
            ...
            raise HTTPException(
                ...
                detail="User does not have permission to delete the slot",
            )
        await self._repository.delete(self._model.id == slot_id)
