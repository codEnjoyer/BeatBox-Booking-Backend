...
class Review(Base):
    ...
    author: Mapped["User"] = relationship(back_populates="reviews", lazy='joined', foreign_keys=[author_id])

    studio_id: Mapped[int] = mapped_column(ForeignKey("studios.id"), nullable=False)
    studio: Mapped["Studio"] = relationship(back_populates="reviews", lazy='joined', foreign_keys=[studio_id])
